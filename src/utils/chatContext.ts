...
/**
 * Pre-fills the in-app chat's input with whatever the user is actually
 * looking at, so starting a chat doesn't mean re-explaining context the app
 * already has — see the AI assistant's system prompt
...
export function buildContextualMessage(params: {
  ...
  if (!hasStartedPlanning) {
    return 'Hola! Estoy viendo las opciones de viaje y tengo una pregunta.'
  }
