 ...
 * optional drill-down list of every expense in the selected category. */
    ...
    <div className="category-layout">
      <div className="category-grid">
        {Object.entries(totalsByCategory).map(([category, amount]) => (
          ...
          >
            <span className="category-big-icon" style={{ background: CATEGORY_META[category as Category].color }}>
              {CATEGORY_META[category as Category].icon}
            </span>
            <span className="category-name">{category}</span>
            <strong>{formatCOP(amount)}</strong>
            <small>
              {days.flatMap(day => day.expenses).filter(expense => expense.category === category).length} movimientos
              <ArrowRight size={15} />
            </small>
          </button>
        ))}
