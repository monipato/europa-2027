...
- **`src/assets/ducks/*.png`** are the app's small decorative "traveling duck" stickers (not the logo, not emoji) — reach for one here before adding an emoji duck or new artwork. They're final, hand-cropped, checked-in assets; there is no generator script or source sheet in the repo for them anymore. They came from two source sheets, both deleted once fully cropped (at the user's request, to avoid keeping large unused source images around):
  ...
  Both sheets' cards have a baked-in illustrated background (not transparent), unlike the first sheet's cutout stickers — style these as small rounded-square badges, not rotated transparent stickers. If a new duck crop is ever needed, it has to be sourced from a freshly re-saved sheet — there's nothing left in the repo to re-slice.
